<template>
  <div class="page">
    <div class="toolbar">
      <h3>平台列表</h3>
      <el-button type="primary" @click="showDialog('add')">新增平台</el-button>
    </div>
    ...
    <!-- 新增/编辑 -->
    <el-dialog v-model="showAddDialog" :title="editMode === 'add' ? '新增平台' : '编辑平台'" width="500px">
      <el-form :model="form" label-width="80px">
        <el-form-item label="平台名称"><el-input v-model="form.name" placeholder="如：天猫" /></el-form-item>
        <el-form-item label="编码"><el-input v-model="form.code" placeholder="如：tmall" /></el-form-item>
...
async function loadData() {
  loading.value = true
  try {
    const res = await baseDataApi.getPlatforms()
    platforms.value = res.data?.data || res.data || []
...
async function saveItem() {
  try {
    if (editMode.value === 'edit') {
      await baseDataApi.updatePlatform(editingId.value, form.value)
      ElMessage.success('更新成功')
    } else {
      await baseDataApi.addPlatform(form.value)
...
async function deleteItem(row) {
  try {
    await ElMessageBox.confirm(`确认删除平台 "${row.name}"？`, '确认删除', { type: 'warning' })
    await baseDataApi.deletePlatform(row._id)
